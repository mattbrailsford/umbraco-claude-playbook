...
description: >-
  ...
  — a one-or-two-sentence "when to use" plus a short idiomatic snippet per pattern, primarily
  in C#. A closing section gives full Lit/TypeScript examples for the patterns that come up
  often in backoffice component work (State, Strategy, Command, Composite) and a one-line
  frontend pointer for the rest — the "when to use" column applies to either layer. Use when
  choosing or reviewing a design pattern in an Umbraco package, or answering "which pattern
  fits here" — not as a checklist to force into every class or component.
---
...
The same 23 patterns apply on the frontend — the "when to use" column above doesn't change,
only the syntax does. Full treatment for the ones that come up often in backoffice component
work; a one-line pointer for the rest rather than doubling every section for parity's sake.
...
Same "when to use" as the C# table above; real, but less frequent in typical backoffice
...
- **Iterator, Builder, Factory Method, Abstract Factory, Bridge, Proxy, Mediator, Memento** —
  translate directly from the C# examples above; nothing frontend-specific changes the shape
  enough to warrant a separate illustration.
- **Flyweight, Interpreter, Visitor, Singleton, Prototype** — genuinely rare in a typical
  backoffice component. Reach for the C# guidance above if the same need shows up on the
